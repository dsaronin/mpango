...
class Subscription < ActiveRecord::Base
  ...
  def send_invoice(amount)
    Mpango.mailer.deliver_invoice(subscribable, self, amount)
  end
  ...
  def expire_after_grace!
    self.expire_on = [Date.today, paid_through].max + Mpango.days_grace
    Mpango.activity_log[self] << "now set to expire on #{self.expire_on}" if Mpango.log?
    Mpango.mailer.deliver_expiration_warning(subscribable, self)
    save!
  end

  # sends an expiration email, then downgrades to a free plan
  def expire!
    Mpango.activity_log[self] << "expired!" if Mpango.log?
    Mpango.mailer.deliver_expiration_notice(subscribable, self)
    # downgrade to a free plan
    self.subscription_plan = Mpango.expired_plan
  ...
  def credit_card=(cc)
    response = (billing_key) ? Mpango.gateway.update(billing_key, cc) : Mpango.gateway.store(cc)
    raise Mpango::CreditCardStorageError.new(response.message) unless response.success?
  ...
  def receive_payment(value)
    ...
    Mpango.activity_log[self] << "now paid through #{self.paid_through}" if Mpango.log?
  ...
  def cancel_in_remote_system
    Mpango.gateway.cancel(self.billing_key)
  end
end