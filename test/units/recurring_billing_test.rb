...
class RecurringBillingTest < Test::Unit::TestCase
  ...
  class Subscription < ::Subscription
    include Mpango::RecurringBilling
  end
  ...
  def test_run_billing_sends_report
    ...
    Mpango.stubs(:admin_report_recipients).returns("test@example.com")

    Mpango.mailer.expects(:deliver_admin_report)
    Subscription.run_billing
  end
  ...
  def test_processing_new_transactions
    ...
    t = Mpango::Transaction.new(:billing_key => subscription.billing_key, :amount => subscription.subscription_plan.rate, :success => true)
  ...
  def test_processing_a_failed_transaction
    subscription = subscriptions(:bobs_subscription)
    paid_through = subscription.paid_through
    t = Mpango::Transaction.new(:billing_key => subscription.billing_key, :amount => subscription.subscription_plan.rate, :success => false)
