...
class SubscriptionTest < Test::Unit::TestCase
  ...
  def test_instance_expire
    Mpango.expired_plan = subscription_plans(:free)
    Mpango.gateway.expects(:cancel).once.returns(nil)
    ActionMailer::Base.deliveries = []
  ...
  def test_expire_after_grace
    ...
    subscriptions(:bobs_subscription).expire_after_grace!
    assert_equal Date.today + Mpango.days_grace, subscriptions(:bobs_subscription).reload.expire_on
  end

  def test_expire_after_grace_with_remaining_paid_period
    subscriptions(:bobs_subscription).paid_through = Date.today + 1
    subscriptions(:bobs_subscription).expire_after_grace!
    assert_equal Date.today + 1 + Mpango.days_grace, subscriptions(:bobs_subscription).reload.expire_on
  end

  def test_grace_and_expiration
    assert_equal 3, Mpango.days_grace, "test assumption"
  ...
  def test_adding_a_credit_card
    ...
    cc = Mpango::CreditCard.new
    response = Mpango::Response.new(true)
    response.billing_key = "alphabravo"
    Mpango.gateway.expects(:store).with(cc).returns(response)
  ...
  def test_updating_a_credit_card
    ...
    Mpango.gateway.expects(:update).with(subscription.billing_key, cc).returns(response)
  ...
  def test_failing_to_add_a_credit_card
    subscription = Subscription.new
    cc = Mpango::CreditCard.new
    response = Mpango::Response.new(false)
    Mpango.gateway.expects(:store).returns(response)

    assert_raises Mpango::CreditCardStorageError do subscription.credit_card = cc end
  end
